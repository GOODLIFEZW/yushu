...
def redraw_drift(did):
    ...
    with db.auto_commit():
        ...
        # PendingStatus是枚举类型，不能直接赋值
        # 1 PendingStatus.Redraw.value （麻烦）
        # 2 直接赋值4 （不好）
        # 3 drift model下定义getter setter （推荐）
        drift.pending = PendingStatus.Redraw
        current_user.beans += 1
    return redirect(url_for('web.pending'))  # 这里使用ajax更好
